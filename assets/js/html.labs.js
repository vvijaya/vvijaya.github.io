window.defer.push(() => {
    ...
    w.on([w.dz.ctrl, w.dz.face], "drop dragover dragend dragleave change", (e) => {
        w.dz.fileHandler(
            ...
            (F) => {
                w.dz.face.innerHTML = w.dz.face.innerHTML.indexOf("</div>") < 0 ? "" : w.dz.face.innerHTML;
    ...
    w.updateGallery = (G, idx = 1) => {
        ...
        last = list.length - 1;
        idx = idx === Number(idx) ? idx + (Number(G.dataset.idx) || 0) : Number(idx) || 0;
        idx = idx < 0 ? last : idx;
        idx = idx > last ? 0 : idx;
        ...
        if (!newImg) {
            ...
            w.on(newImg, "load", ((oldImg, newImg) => {
                return () => {
    ...
    w.on(w.all(".gallery .prev, .gallery .next"), "click", (e) => {
        ...
        w.tmp = w.hasClass(e.target, "prev") ? w.updateGallery(G, -1) : w.updateGallery(G, 1);
