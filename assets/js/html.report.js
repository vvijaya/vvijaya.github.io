...
<label for='swap_rows_cols_${w.md5(F.name)}' class='swap-rows-cols '>
    ...
    <span> ${F.reportObjFlip ? 'Swap' : 'Can\'t swap'} the rows &amp; columns</span>
</label>
        ...
        <span class='data-list'>
                            ...
                            for (const key in obj) {
                                if (w.Reflect.has(obj, key)) {
                                    outHTML += w.marked(`**${smartlink(key)}** : ${smartlink(obj[key])}`)
                                        .split('href=')
        ...
        w.dz.fileHandler(
            ...
            (F) => {
                ...
                w.reportModal = new w.Modal({
                    ...
                    header: 'Loading',
                    body: `<div id="nprogress"><div class="spinner" role="spinner"><div class="spinner-icon">${
                        ''
                    }</div></div></div>`
                })
